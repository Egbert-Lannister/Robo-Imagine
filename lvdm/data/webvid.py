...
class BaseDataset(Dataset, ABC):
    """
    Base Dataset class defining the API.
    Child classes must implement the required methods.
    """

    @abstractmethod
    def __getitem__(self, index):
        """Return a single sample: video frames + metadata."""
        pass
...
class RobotDataset(BaseDataset):
    ...
    def _random_sample_indices(self, length):
        # 先随机取 15 个不重复的数字，从 0 到 length-1
        nums = random.sample(range(0, length), 15)
        # 添加 length
        nums.append(length)
        # 排序
        nums = sorted(nums)
    ...
    def _random_start_sample_indices(self, length):
        if length < self.video_length - 1:
            raise ValueError(f"length must be at least {self.video_length}")
        
        # Step 1: 随机选取 start
        start = random.randint(0, length - self.video_length + 1)
        print(f"Random start: {start}")
        
        # Step 2: 在 [start, length-1] 取 15 个不重复数字
        nums = random.sample(range(start, length), self.video_length - 1)
    ...
    def __getitem__(self, index):
        ...
        while True:
            ...
            # frame_indices = self._random_start_sample_indices(frame_num - 1)
...
class WebVid(BaseDataset):
    ...
    def _get_video_path(self, sample):
        ...
        rel_video_fp = os.path.join(sample['page_dir'], str(sample['videoid']))
